...
export const createBook = async (req, res) => {
  try {
    const { name, description, price, cost, isbn, author, publisher, category, classLevel, subject, language, stockQty, coverImage } = req.body;

    if (!name || !author) {
      return res.status(400).json({ success: false, message: "Missing required fields: name and author are required." });
    }
    ...
    // } else if (typeof filesObjOrArray === "object" && filesObjOrArray !== null) {
    ...
    const images = files
      .map((file, idx) => {
        const url = file.path || file.secure_url || file.url || file.location || (file?.uploads && file.uploads?.[0]?.path) || null;
        const publicId = file.filename || file.public_id || file.publicId || file.key || null;
    ...
    const finalCoverImage = coverImage || (images.length ? images[0].url : undefined);
    ...
    return res.status(201).json({ success: true, message: "Book created successfully", book: created });
  } catch (err) {
...
export const updateBook = async (req, res) => {
  try {
    ...
    const safeTrim = (v) => (v === undefined || v === null ? undefined : String(v).trim());

    const { name, description, price, cost, isbn, author, publisher, category, classLevel, subject, language, stockQty, coverImage, removeImagePublicIds } = req.body;

    let categoryId = undefined;
    if (category !== undefined && category !== null && String(category).trim() !== "") {
      if (mongoose.Types.ObjectId.isValid(category)) {
        const found = await Category.findById(category).exec();
        if (!found) return res.status(400).json({ success: false, message: "Category id provided does not exist." });
        categoryId = found._id;
      } else {
        const nameTrim = String(category).trim();
        let existing = await Category.findOne({ name: nameTrim }).exec();
        if (!existing) {
          try {
            existing = await Category.create({ name: nameTrim });
          } catch (err) {
            if (err.code === 11000) {
              existing = await Category.findOne({ name: nameTrim }).exec();
            } else {
              throw err;
            }
          }
    ...
    if (Array.isArray(filesObjOrArray)) {
      ...
      for (const key of Object.keys(filesObjOrArray)) {
        if (Array.isArray(filesObjOrArray[key])) files = files.concat(filesObjOrArray[key]);
      }
    }
    ...
    const newImages = files
      .map((file, idx) => {
        ...
        const publicId = file.filename || file.public_id || file.publicId || file.key || null;
        if (!url) return null;
        return { url, publicId, position: book.images.length + idx + 1 };
      })
      .filter(Boolean);
      ...
      try {
        if (typeof removeImagePublicIds === "string") {
          ...
          if (trimmed.startsWith("[") || trimmed.startsWith("{")) {
            ...
            removeIds = trimmed.split(",").map((s) => s.trim()).filter(Boolean);
          }
        } else if (Array.isArray(removeImagePublicIds)) {
          removeIds = removeImagePublicIds;
        }
      } catch (err) {
        removeIds = String(removeImagePublicIds).split(",").map((s) => s.trim()).filter(Boolean);
      }
    }
    ...
    if (removeIds.length > 0) {
      ...
      const toRemove = book.images
        .filter((img) => img.publicId && removeIds.includes(String(img.publicId)))
      ...
      if (toRemove.length > 0) {
        ...
        const deletionPromises = toRemove.map((publicId) =>
          cloudinary.uploader
            ...
            .then((res) => ({ publicId, status: "fulfilled", result: res }))
            .catch((err) => ({ publicId, status: "rejected", error: err?.message || err }))
        );
        ...
        book.images = book.images.filter((img) => !img.publicId || !removedSet.has(String(img.publicId)));
      }
    }
    ...
    if (coverImage !== undefined && coverImage !== null && String(coverImage).trim() !== "") {
      ...
      const uploadedCoverFile = (req.files && req.files.coverImage && req.files.coverImage[0]) || null;
