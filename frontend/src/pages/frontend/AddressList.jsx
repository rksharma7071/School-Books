...
function AddressList() {
    ...
    const deleteAddress = async (id) => {
        if (!window.confirm("Do you want to delete this address?")) return;
        ...
        try {
            ...
            revalidator.revalidate();   // ← re-runs getProfile loader
        } catch (error) {
            setToastConfig({
                type: "error",
                message:
    ...
    const list = Array.isArray(address) ? address : [];
    ...
    return (
        <div>
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
                ...
                <Link
                    to="new"
                    className="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-md bg-blue-600 text-white text-sm font-medium hover:bg-blue-700 transition"
                >
                    + Add New
                </Link>
            </div>
