...
function EditBook() {
    ...
    useEffect(() => {
        ...
        setOptions(loadedOptions.length ? loadedOptions : [{ name: "class", values: [""] }]);
    ...
    // console.log("loadedBook.variants: ",loadedBook.variants[0].price);
    // console.log("loadedBook.variants: ", loadedBook);
    ...
    const addOptionValue = (optIndex) => {
        setOptions((prev) =>
            prev.map((opt, i) =>
                i === optIndex ? { ...opt, values: [...option.values, ""] } : opt
            )
        );
    };
    ...
    const removeOptionValue = (optIndex, valIndex) => {
        setOptions((prev) =>
            prev.map((opt, i) => {
                ...
                return {
                    ...opt,
                    values: option.values.filter((_, vi) => vi !== valIndex),
                };
            })
        );
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await axios.put(
                `${import.meta.env.VITE_API}/api/product/${loadedBook._id}`,
                formData,
                {
                    headers: {
            ...
            setToastConfig({
                type: "error",
                message: error.response?.data?.message || error.message || "Failed to update book.",
            });
            setShowToast(true);
        }
    };
    ...
    if (!loadedBook) {
        return (
            <div className="max-w-7xl mx-auto py-12 text-center text-gray-500">Loading book…</div>
        );
    }
    ...
    return (
        <div className="max-w-7xl mx-auto space-y-4">
            <div className="mb-4">
                ...
                <p className="text-sm text-gray-500">Update the book details, options, and variants below.</p>
            </div>

            <div className="rounded-xl border border-gray-200 bg-white overflow-hidden">
                <div className="overflow-x-auto">
                    <form onSubmit={handleSubmit} className="max-w-6xl mx-auto bg-white p-6 space-y-6">
                                            ...
                                            {variants.map((variant, vi) => (
                                                <tr key={vi} className="border-b border-gray-200">
                                                    <td className="py-1 pr-3 text-gray-700">
                                                        {Object.entries(variant.options).map(([k, val]) => `${k}=${val}`).join(", ")}
                                                    </td>
